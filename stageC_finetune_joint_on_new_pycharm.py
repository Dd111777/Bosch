...
def main():
    ...
    ap.add_argument("--seed_repeats", type=int, default=3)
    ap.add_argument("--trials", type=int, default=700)
    ap.add_argument("--max_drop", type=int, default=10)
    ...
    ap.add_argument("--mode", type=str, default="both", choices=["screen", "full", "both"])
    ap.add_argument("--seed_start", type=int, default=2026)     # split_seed 起点
    ap.add_argument("--split_trials", type=int, default=700)    # split 枚举数
    ap.add_argument("--drop_list", type=str, default="0,7,10")  # 每个 split 的 drop 版本
    ap.add_argument("--screen_epochs", type=int, default=50)
    ap.add_argument("--screen_patience", type=int, default=10)
    ap.add_argument("--top_k", type=int, default=5)            # screen 后选 TopK dataset 进 full
    ...
    # -----------------------------------------------------------
    # 3) Find / Save best common split
    # -----------------------------------------------------------
    drop_list = [int(x) for x in str(args.drop_list).split(",") if str(x).strip() != ""]
    datasets_all = iter_candidate_datasets(
        raw=raw,
        key_recipes=key_recipes,
        families_eval=families_eval,
        test_ratio=args.test_ratio,
    ...
    print(f"[Info] Candidate datasets generated = {len(datasets_all)}  (valid splits × drop variants)")
    if len(datasets_all) == 0:
        raise RuntimeError("No valid datasets generated. Try lower min_test_points or adjust val_ratio.")
    ...
    # Ranking（你 best 里出现过 Rank_ra0.2）
    for ra in [0.1, 0.2]:
        add_exp(ExpCfg(
            name=f"Rank_ra{ra}",
    ...
    # Time-weighting（你 best 里出现过 Timew_late_0.2 等）
    ...
    # T3 Clamp（你 best 里出现过 T3_Clamp_c5.0_b0.5）
    ...
    # Mixup（你 best 里 d1 的 Mixup_a0.4 很强）
    ...
    # summary paths（screen/full 共用同一 summary_all，方便统一排序）
    ...
    df_screen = df_all[df_all["phase"].astype(str) == "screen"].copy()
    ds_rank = compute_best_minr2_per_dataset_from_summary(df_screen, families_eval=families_eval)
    ds_rank.to_csv(os.path.join(args.out_dir, "dataset_ranking_best_minr2.csv"), index=False)

    top_k = int(args.top_k)
    top_rows = ds_rank.head(top_k)
    print("\n[TOP DATASETS] by best_minR2")
    print(top_rows)

    # 反查构造 top datasets（用 split_seed + drop_tag 去 datasets_all 里找）
    top_set = set((int(r.split_seed), str(r.drop_tag)) for r in top_rows.itertuples(index=False))
    datasets_top = []
    for sp in datasets_all:
        if (int(sp.get("seed", -1)), str(sp.get("drop_tag", "drop0"))) in top_set:
            datasets_top.append(sp)

    # -----------------------------
    # Phase 2: FULL (TopK 数据集上，跑 full epoch)
    # -----------------------------
    ...
    # -----------------------------
    # Build summary_best: best across seeds for each (dataset, family, exp)
    # -----------------------------
    df_all = pd.read_csv(summary_all)
